...
class App extends Component {
  ...
  clickOnImg = thumbnailUrl => {
    const {imgUrl} = this.state
    const selectedImage = imagesList.filter(
      image => image.thumbnailUrl === thumbnailUrl,
    )
    console.log(selectedImage)
    const {imageUrl} = selectedImage[0]
    if (imageUrl !== imgUrl) {
      clearInterval(this.timerId)
      this.setState({isSelected: true})
    } else {
  ...
  render() {
    const {score, time, imgUrl, isSelected, selectedTab} = this.state
    const filteredImageList = imagesList.filter(
      eachdetails => eachdetails.category === selectedTab,
    )
    return (
        ...
        <div className="header-container">
          {isSelected ? (
            ...
            <div className="first-div">
              <img src={imgUrl} alt="match" className="selected-img" />
              <ul className="tab-items">
                {tabsList.map(eachdetails => (
                  <li key={eachdetails.tabId}>
                    <button
                      type="button"
                      className={`btn-img ${
                        selectedTab === eachdetails.tabId ? 'selected-tab' : ''
                      } `}
                      onClick={() => this.tofindTab(eachdetails.tabId)}
                    >
                      {eachdetails.displayText}
                    </button>
                  </li>
                ))}
              </ul>
              <ul className="image-tab">
                {filteredImageList.map(eachImage => (
                  <li key={eachImage.id}>
                    <button
                      type="button"
                      className="img-btn"
                      onClick={() => this.clickOnImg(eachImage.thumbnailUrl)}
                    >
                      <img
                        className="img"
